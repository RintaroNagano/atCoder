...
func (t *takahashi) proceed(direction byte) {
	switch direction {
	case UP:
		t.y--
	case RIGHT:
...
func isGround(x, y int, grid [][]byte) bool {
	return grid[y][x] != '#'
}
...
func solve() {
	...
	s := GetNextNLineChars(h)

	//	fastio.Println(h, w, t, s)
	...
	for i := 1; i < h-1; i++ {
		for j := 1; j < w-1; j++ {
			...
			if !isGround(tk.x, tk.y, s) {
				continue
			}
			for count, direction := range t {
				// fastio.Println(*&tk.x, tk.y, string(direction), len(t), count)
				tk.proceed(direction)
				if !isGround(tk.x, tk.y, s) {
					break
				}
				if count == len(t)-1 {
