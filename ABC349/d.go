...
func (i *FastIo) GetNextUint64() uint64             { return i.Atoui64(i.Text()) }
func (i *FastIo) Atoui64(s string) uint64           { x, _ := strconv.ParseUint(s, 10, 64); return uint64(x) }
...
func solve() {
	l := fastio.GetNextUint64()
	r := fastio.GetNextUint64()

	ans := make([]uint64, 10000)
	fastio.Println("findGoodRange start")

	findGoodRange(l, r, ans, uint64(0))
...
func findLargestPowerOfTwo(x uint64) uint64 {
	if x == 0 {
		return 1
	}
...
func findGoodRange(l uint64, r uint64, ans []uint64, prevR uint64) {
	fastio.Println("findGoodRange start")
	fastio.Println(l, r, ans, prevR)
	tmpAns := make([]uint64, 1)
	R := findLargestPowerOfTwo(r)
	tmpL := R
	for l < tmpL/2 {
		tmpL = R / 2

		tmpAns = append([]uint64{tmpL}, tmpAns...)
	}
	tmpAns = append([]uint64{l}, tmpAns...)
