...
func solve() {
	...
	searchSlicePartFromSum := func(from, sum int) (exist bool, to int) {
		val := 0
		exist = false
		to = -1
		length := len(a)
		for i := from; val < sum && i < length; i++ {
			val += a[i]
			if val == sum {
				exist = true
				to = i + 1
	...
	for i := 0; i < n; i++ {
		x := i
		exist, y := searchSlicePartFromSum(x, p)
		if exist {
			exist, z := searchSlicePartFromSum(y, q)
			if exist {
				exist, _ := searchSlicePartFromSum(z, r)
				if exist {
					fastio.Println("Yes")
					return
				}
