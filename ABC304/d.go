...
func solve() {
	...
	for fori := 0; fori < n; fori++ {
		indexX := sort.Search(A, func(i int) bool { return a[i] >= pos[fori][0] })
		indexY := sort.Search(B, func(i int) bool { return b[i] >= pos[fori][1] })
		m[int64(indexX)+int64(indexY)*200000] += 1
	}
	...
	// fastio.Println("m: ", m)
	for _, v := range m {
		// fastio.Println("min: ", minIchigo, ", max: ", maxIchigo)
		maxIchigo = max(maxIchigo, v)
		minIchigo = min(minIchigo, v)
	}

	fastio.Println(minIchigo, maxIchigo)
}
