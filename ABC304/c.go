...
func solve() {
	n := fastio.GetNextInt()
	d := fastio.GetNextFloat64()

	pos := GetNextNMInt(n, 2)
	isInfected := make([]bool, n)
	isInfected[0] = true
	var infectedQue []int
	infectedQue = append(infectedQue, 0)

	for len(infectedQue) > 0 {
		// fastio.Println(infectedQue)
		i := infectedQue[0]
		infectedQue = infectedQue[1:]
		for j := 0; j < n; j++ {
			if distance(pos[i][0], pos[i][1], pos[j][0], pos[j][1]) <= d {
				// 感染が既に判明していたひとはキューイングされない
				if !isInfected[j] {
					infectedQue = append(infectedQue, j)
				}
				isInfected[j] = true
			}
		}
