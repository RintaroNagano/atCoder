...
func NewFastIo(fp io.Reader, wfp io.Writer) *FastIo {
	...
	return &FastIo{Scanner: scanner, Reader: reader, Writer: bufio.NewWriter(wfp)}
}
...
func solve() {
	...
	card := make(map[int][]int, 200001)
	isInBox := make([]map[int]bool, 200001)
	for i := 0; i < 200001; i++ {
		isInBox[i] = make(map[int]bool)
	}

	for iq := 0; iq < q; iq++ {
		query := fastio.GetNextInt()
		switch query {
		case 1:
			i := fastio.GetNextInt()
			j := fastio.GetNextInt()
		...
		case 2:
			i := fastio.GetNextInt()
			sort.Ints(b[i])
			for count := 0; count < len(b[i]); count++ {
				fastio.Printf("%d ", b[i][count])
			}
			fastio.Println()
		case 3:
			i := fastio.GetNextInt()
			sort.Ints(card[i])
			for _, v := range card[i] {
				fastio.Printf("%d ", v)
			}
			fastio.Println()
		}
