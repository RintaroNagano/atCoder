...
func GetNextNIntFrom1(n int) []int {
	a := make([]int, n)
	for i := 0; i < n; i++ {
		a[i] = fastio.GetNextInt()
	}
...
func GetNextNMIntFrom1(n int, m int) [][]int {
	a := make([][]int, n)
	for i := 0; i < n; i++ {
		a[i] = GetNextNIntFrom1(m)
	}
	a = append([][]int{{}}, a...)
	return a
}
...
func solve() {
	n := fastio.GetNextInt()
	a := GetNextNMIntFrom1(n, n)
	b := GetNextNMIntFrom1(n, n)
	//fastio.Println(a, b)
