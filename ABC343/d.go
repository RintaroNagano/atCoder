...
func solve() {
	...
	diversity := make([]int, t+1)
	countPerNumber := make(map[int]int)
	scores := make([]int, n+1)
	countPerNumber[0] = n
	diversity[0] = 1
	...
	for i := 0; i < t; i++ {
		...
		nextScore := scores[a] + b
		scores[a] = nextScore

		diff_diversity := 0
		// 種類減った？
		countPerNumber[currentScore]--
		if countPerNumber[currentScore] == 0 {
			diff_diversity--
		}

		// 種類増えた？
		countPerNumber[nextScore]++
		if countPerNumber[nextScore] == 1 {
			diff_diversity++
		}
		fastio.Println(diff_diversity + diversity[i])
		diversity[i+1] = diff_diversity + diversity[i]
	}
}
