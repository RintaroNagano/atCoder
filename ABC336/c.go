...
func solve() {
	...
	goodInt := findGoodInt(n)
	fastio.Println(goodInt)
}

// findGoodInt は、与えられたNに対して、N番目の良い整数を返します。
func findGoodInt(N int64) string {
	// 5進数表現を構築
	base5 := strconv.FormatInt(N-1, 5)

	// 5進数表現の各桁を対応する偶数に置き換え
	result := ""
	for _, digit := range base5 {
		// '0'から'4'までの文字を、対応する偶数の文字に変換
		evenDigit := (digit-'0')*2 + '0'
		result += string(evenDigit)
	}
