...
func solve() {
	...
	tiles := make([][2]int, n)
	for i := 0; i < n; i++ {
		tiles[i][0] = fastio.GetNextInt()
		tiles[i][1] = fastio.GetNextInt()
	}
	if canCoverAll(h, w, tiles) {
		fmt.Println("Yes")
	} else {
		fmt.Println("No")
	}
...
func canCoverAll(h, w int, tiles [][2]int) bool {
	board := make([][]bool, h)
	for i := range board {
		board[i] = make([]bool, w)
	}
	return dfs(board, tiles, 0)
}

func dfs(board [][]bool, tiles [][2]int, index int) bool {
	if index == len(tiles) {
		return isAllCovered(board)
	}
	for i := 0; i < len(board); i++ {
		for j := 0; j < len(board[0]); j++ {
			for _, rotation := range [][2]int{{tiles[index][0], tiles[index][1]}, {tiles[index][1], tiles[index][0]}} {
				if canPlaceTile(board, i, j, rotation[0], rotation[1]) {
					placeTile(board, i, j, rotation[0], rotation[1], true)
					if dfs(board, tiles, index+1) {
						return true
					}
					placeTile(board, i, j, rotation[0], rotation[1], false)
				}
			}
		}
	}
...
func canPlaceTile(board [][]bool, x, y, h, w int) bool {
	if x+h > len(board) || y+w > len(board[0]) {
		return false
	}
	for i := x; i < x+h; i++ {
		for j := y; j < y+w; j++ {
...
func placeTile(board [][]bool, x, y, h, w int, fill bool) {
	for i := x; i < x+h; i++ {
		for j := y; j < y+w; j++ {
			board[i][j] = fill
		}
...
func isAllCovered(board [][]bool) bool {
	for i := range board {
		for j := range board[i] {
			if !board[i][j] {
				return false
