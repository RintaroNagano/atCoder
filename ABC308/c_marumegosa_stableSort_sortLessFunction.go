...
func solve() {
	...
	c_rates := make([]c_rate, n)

	for i := 0; i < n; i++ {
		c_rates[i].index = i
	}

	sort.SliceStable(c_rates, func(i, j int) bool {
		return int64(ab[c_rates[i].index][0])*int64(ab[c_rates[j].index][0]+ab[c_rates[j].index][1]) > int64(ab[c_rates[j].index][0])*int64(ab[c_rates[i].index][0]+ab[c_rates[i].index][1])
	})

	// これはだめ，多分，c_rate[i] が c_retes[j]　より前に来るかどうかを決める関数に，c_rates[i]とc_rates[j]が関わってこないから，うまくソートできてない．
	// sort.SliceStable(c_rates, func(i, j int) bool {
	// 	return int64(ab[i][0])*int64(ab[j][0]+ab[j][1]) > int64(ab[j][0])*int64(ab[i][0]+ab[i][1])
	// })

	for _, v := range c_rates {
		fastio.Printf("%d ", v.index+1)
	}
	fastio.Printf("\n")
