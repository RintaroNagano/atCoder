...
func solve() {
	...
	// 長さ2の経路の数を数える
	pathCount := 0
	counted := make(map[int][]int, n+1)

	for x := 1; x <= n; x++ {
		for y, _ := range friends[x] {
			for z, _ := range friends[y] {
				if z != x && !slices.Contains(friends[x], y) && !slices.Contains(counted[x], y) {
					// XとZは友達ではなく、XとY、YとZは友達
					pathCount++
					// 友人関係の追加
					friends[x] = append(friends[x], z)
					friends[z] = append(friends[z], x)
					// すでに数えた友人関係を追加
					counted[x] = append(counted[x], z)
					counted[z] = append(counted[z], x)
					// fastio.Println(x, z)
				}
			}
		}
